      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="text-[var(--text-secondary)] text-sm mt-1">
            Upload your notes and CampusMind will index them into your academic memory
          </motion.p>
        </div>
      </div>
          ...
          >
            <GlassCard glow className="p-10 text-center">
              ...
              <p className="text-[var(--text-secondary)] text-sm mb-6">
                {files.length} file{files.length !== 1 ? 's' : ''} processed and added to your memory
              </p>
          ...
          >
            <div
              ...
              className={cn(
                ...
                dragActive
                  ? 'border-[var(--accent)] bg-[#1a73e8]/5'
                  : 'border-[var(--border-primary)] hover:border-[var(--border-glow)] hover:bg-white/[0.02]'
              )}
            >
              <input
                ref={inputRef}
                type="file"
                multiple
                accept=".pdf,.docx,.txt,.md,.pptx,.png,.jpg,.jpeg"
                onChange={handleFileSelect}
                className="hidden"
              />
              <div className={cn(
                'w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 transition-all',
                dragActive ? 'bg-[#1a73e8]/20 scale-110' : 'bg-white/[0.04]'
              )}>
                <Upload className={cn(
                  'w-8 h-8 transition-colors',
                  dragActive ? 'text-[var(--accent-light)]' : 'text-[var(--text-muted)]'
                )} />
              </div>
              <h3 className="font-display text-lg font-bold mb-1">
                {dragActive ? 'Drop your files here' : 'Drag & drop your notes'}
              </h3>
              <p className="text-sm text-[var(--text-muted)] mb-4">
                PDF, DOCX, TXT, MD, PPTX &mdash; up to 50MB each
              </p>
              <button className="btn-secondary text-sm !pointer-events-auto">
                Browse files
              </button>
            </div>
              ...
              >
                <GlassCard glow className="p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={() => setFiles([])} className="btn-ghost text-xs !p-1">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                    ...
                    {files.map((f, i) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-[var(--text-muted)]">{(f.size / 1024).toFixed(0)} KB</div>
                        </div>
                        <button onClick={() => setFiles(prev => prev.filter((_, j) => j !== i))} className="text-[var(--text-muted)] hover:text-[var(--error)]">
                          <X className="w-4 h-4" />
                        </button>
                      </motion.div>
                    ))}
